// =========================================================================
// MODE I DRIVER (double cantilever beam)
// =========================================================================
...
{
	// ---------------------------------------------------
	// 1. PARAMETERS (Specific to Mode I DCB)
	// ---------------------------------------------------
	...
	double a_min = mp.a0 + 1e-3; // minimum crack length before dPi/dA fitting is started
	
	// material properties
	double E_int = 126.e9; // interface Young's modulus
	double nu_int = 0.3;  // interface Poisson's ratio
	double Gc_int = 281.; // interface fracture toughness
	...
	for(double D = Dinit; D <= Dend + Dinc/10; D += Dinc)
	{
		...
		while(true)
		{
			...
			if(R_normalised < tol)
			{
				...
				// Gc Calculation (Mode I Specific)
